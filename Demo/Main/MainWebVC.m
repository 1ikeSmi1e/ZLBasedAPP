...
- (void)loadWithUrl:(id)responseObject{
    if (!self.isSuccess) {
        NSString *navColorStr = responseObject[@"colorStr"];
        NSScanner *scanner = [NSScanner scannerWithString:navColorStr];
        unsigned hexNum;
        [scanner scanHexInt:&hexNum];
        self.stautsView.backgroundColor = UIColorFromRGB(hexNum);
        
        NSURLRequest *request = [NSURLRequest requestWithURL:[NSURL URLWithString:responseObject[@"webUrl"]]];
