...
#define LC_DEFAULT_ANIMATION_DURATION 0.25f
...
+ (instancetype)initWithTitle:(NSString *)title buttonTitles:(NSArray *)buttonTitles redButtonIndex:(NSInteger)redButtonIndex grayButtonIndex:(NSInteger)grayButtonIndex delegate:(id<LCActionSheetDelegate>)delegate {
    
    return [[self alloc] initWithTitle:title buttonTitles:buttonTitles redButtonIndex:redButtonIndex grayButtonIndex:grayButtonIndex delegate:delegate];
}

+ (instancetype)initWithTitle:(NSString *)title buttonTitles:(NSArray *)buttonTitles redButtonIndex:(NSInteger)redButtonIndex grayButtonIndex:(NSInteger)grayButtonIndex clicked:(LCActionSheetBlock)clicked {
    
    return [[self alloc] initWithTitle:title buttonTitles:buttonTitles redButtonIndex:redButtonIndex grayButtonIndex:grayButtonIndex clicked:clicked];
}
...
- (instancetype)initWithTitle:(NSString *)title
                 buttonTitles:(NSArray *)buttonTitles
               redButtonIndex:(NSInteger)redButtonIndex
              grayButtonIndex:(NSInteger)grayButtonIndex
                     delegate:(id<LCActionSheetDelegate>)delegate {
    if (self = [super init]) {
        ...
        self.buttonTitles = [[NSMutableArray alloc] initWithArray:buttonTitles];
        self.redButtonIndex = redButtonIndex;
        self.grayButtonIndex = grayButtonIndex;
        self.delegate = delegate;
    }
...
- (void)setupMainView {
    ...
    if (self.buttonTitles.count) {
        ...
        for (int i = 0; i < self.buttonTitles.count; i++) {
            ...
            }else if (i == self.grayButtonIndex) {
                
                titleColor = LCColor(190.0f, 190.0f, 190.0f);
...
//            NSString *linePath = [bundlePath stringByAppendingPathComponent:@"bgImage_HL@2x.png"];
            UIImage *bgImage = [UIImage imageNamed:@"SQbgImage_HL"];
...
//            UIImage *lineImage = [UIImage imageNamed:@"SQcellLine"];
            
            // 所有线条
            UIImageView *line = [[UIImageView alloc] init];
//            [line setImage:lineImage];
            [line setBackgroundColor:[UIColor colorWithRed:240.0f / 255.0f green:240.0f / 255.0f blue:240.0f / 255.0f alpha:1.0f]];
...
//    NSString *linePath = [bundlePath stringByAppendingPathComponent:@"bgImage_HL@2x.png"];
    ...
    [cancelBtn setFrame:CGRectMake(0, btnY, SCREEN_SIZE.width, BUTTON_H + (SCREEN_SIZE.height==812?30:0))];
    [bottomView addSubview:cancelBtn];
    if (SCREEN_SIZE.height==812) {
        cancelBtn.titleEdgeInsets = UIEdgeInsetsMake(0, 0, 15, 0);
    }
    CGFloat bottomH = CGRectGetMaxY(cancelBtn.frame);//(self.title ? BUTTON_H : 0) + BUTTON_H * self.buttonTitles.count + BUTTON_H + 5.0f;
...
- (void)didClickCancelBtn {
    ...
    !self.clickedBlock?:self.clickedBlock(self.buttonTitles.count);
}
...
- (void)show {
    ...
    self.bottomView.y = SCREEN_SIZE.height;
    [UIView animateWithDuration:self.animationDuration delay:0 options:UIViewAnimationOptionCurveEaseOut animations:^{
        ...
        self.bottomView.y = SCREEN_SIZE.height - _bottomView.height;
...
- (void)dismiss:(UITapGestureRecognizer *)tap {
    ...
    [UIView animateWithDuration:self.animationDuration delay:0 options:UIViewAnimationOptionCurveEaseOut animations:^{
        ...
        _bottomView.y = SCREEN_SIZE.height;
